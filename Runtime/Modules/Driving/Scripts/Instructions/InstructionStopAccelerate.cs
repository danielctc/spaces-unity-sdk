...
{
    [Title("Accelerate Stop")]
    [Description("Accelerate the Car Controller")]

    [Category("Car/Accelerate Stop")]

    [Keywords("Car", "Accelerate", "Stop")]

    [Image(typeof(IconWheel), ColorTheme.Type.Blue)]

    [Serializable]
    public class InstructionStopAccelerate : Instruction
    {
        [SerializeField] private PropertyGetGameObject m_GameObject = new PropertyGetGameObject();

        // PROPERTIES: ----------------------------------------------------------------------------

        public override string Title => $"Stop accelerate on this {this.m_GameObject}";
        ...
        protected override async Task Run(Args args)
        {
            ...
            if (gameObject == null) return;
            {
                ...
                {
                    carController.StopAcceleration();
                }
            }
        }
    }
